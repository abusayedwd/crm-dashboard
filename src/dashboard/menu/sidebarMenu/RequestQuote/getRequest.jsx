...
import { useAllRequestQuery } from "../../../../redux/features/request/allRequest";
import { useDeleteRequestMutation } from "../../../../redux/features/request/deleteRequest";
...
const RequestQ = () => {
  ...
  const {data: requst} = useAllRequestQuery()  
  const [requestDelete, {isLoading}] = useDeleteRequestMutation()
  console.log(requst);
  const deleteRequest = async(id) => {
    try{
     const res = await requestDelete(id).unwrap();
     console.log(res)
    ...
    },
    {
        ...
        render: (_, record) => (
          <div className="flex gap-2 items-center">
            ...
            <p className="font-medium">{record?.name}
      ...
      },
    {
      ...
      render: (_, record) => (
        <div className="flex gap-2 items-center">
          ...
          <p className="font-medium">{record?.email}
          </p>
        </div>
      ),
    },
   ...
   
    {
      ...
      render: (_, record) => (
        <div className="flex gap-2 items-center">
          ...
          <p className="font-medium"> {record?.phone}
     ...
     
    {
      ...
      render: (_, record) => (
        <Space size="middle">
         ...
         <button
            onLoad={isLoading}
            onClick={() => deleteRequest(record?._id)}
    ...
    <div className="">
      ...
      <div className="rounded-t-lg mt-[24px]">
        <div className="flex py-[22px] justify-between items-center">
          <div>
          <p className="text-header">Request A Quote</p>
          </div>
          <div>
